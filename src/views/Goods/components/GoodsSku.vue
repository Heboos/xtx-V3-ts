...
import { GoodsInfo, Spec, SpecValue } from '@/types/data.d'
...
const getPathMap = () => {
  ...
  // 1. 过滤掉库存为0的sku
  if(props.goods.skus) {
    const skus = props.goods.skus.filter((item:any) => item.inventory > 0)
      // console.log(skus)
      // 2. 遍历有效的sku,获取sku的幂集
      skus.forEach((item:any) => {
        const arr = item.specs.map((sub:any) => sub.valueName)
        // console.log(arr)
        // 3. 调用powerSet获取幂集
...
const getSelectedSpec = () => {
  const arr: string[] = []
  props.goods.specs.forEach((item:any) => {
    const temp = item.values.find((sub:any) => sub.selected)
...
const initSelectedSpec = () => {
  ...
  if(props.goods.specs) {
    const sku = props.goods.skus.find((item:any) => item.id === props.skuId)
  // console.log(sku)
  if (sku) {
    // 2. 如果找到，得到sku中选中的那些规格的值
    props.goods.specs.forEach((item:any, index:number) => {
      // 先让所有的规格不选中
      item.values.forEach((sub:any) => (sub.selected = false))
      const select = sku.specs[index].valueName
      const sub = item.values.find((sub:any) => sub.name === select)!
      sub.selected = true
    })
  }
  }
...
<template >
  <div class="goods-sku" v-if="props.goods">
    <!-- 一个规格 -->
